...
class Monkey:
    ...
    def inspect_item(self):
        ...
        while len(self._items) > 0:
            ...
            # print(f"\tMonkey inspects an item with worry level of {old_worry_level}")
            
            operation = self.operation.replace("old", str(old_worry_level))
            new_worry_level = eval(operation)
            # print(f'\t  New worry level is {new_worry_level} -> {operation}')

            new_worry_level = float(new_worry_level)
            new_worry_level = math.floor(new_worry_level / 3)
            new_worry_level = int(new_worry_level)
            # print(f"\t  Worry level is divided by 3: {new_worry_level}")
            
            outcome = new_worry_level % self.test_value
            outcome = outcome == 0
            # print(f'\t  Current worry level is{" not" if not outcome else ""} divisible by {self.test_value}.')

            dest_monkey = self.dest_monkeys[outcome]
            # print(f'\t  Item with worry level of {new_worry_level} is thrown at Monkey {dest_monkey}')
...
def main(path: str) -> int:
    ...
    throwing_round = 0 
    while throwing_round < 20:
        for monkey in monkeys.values():
            # print(f'Monkey {monkey.name}:')
            for item, dest_monkey in monkey.inspect_item():
                monkeys[dest_monkey].add_item(item)

        # print(f"Round {throwing_round}")        
        # for monkey_name, monkey in monkeys.items():
        #     print(f'{monkey_name=}, items={monkey.items}')
